...
static int rkvdec_init(struct mpp_dev *mpp)
{
	...
	if (ret) {
		mpp_err("failed on clk_get clk_hevc_cabac, fallback to clk_cabac\n");
		dec->hevc_cabac_clk_info = dec->cabac_clk_info;
	}
	/* Set default rates */
	...
	if (!dec->rst_hevc_cabac) {
		mpp_err("No hevc cabac reset resource define, fallback to cabac\n");
		dec->rst_hevc_cabac = dec->rst_cabac;
	}
